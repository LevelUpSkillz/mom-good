...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      const activeVariantIds = product.variants.map((variant) => variant.providerVariantId);
      ...
      if (activeVariantIds.length > 0) {
        await client.query(
          `delete from pod_variants
           where product_id = $1
             and not (provider_variant_id = any($2::text[]))`,
      ...
      return NextResponse.json({
        ...
        variantsImported: activeVariantIds.length,
